...
<template>
    <div>
        <div v-if="welcomeName === ''">
            <Welcome @proceed="onDisplayBoard" />
        </div>
        <div v-else class="dndflow panel" @drop="onDrop">
            <Sidebar />
            <VueFlow v-model="elements" :class="{ dark }" class="basicflow customnodeflow" :default-viewport="{ zoom: 1.5 }"
                :min-zoom="0.2" :max-zoom="4" @dragover="onDragOver" :node-types="nodeTypes">
